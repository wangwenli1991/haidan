...
;(function ($, window) {
    ...
    function _build($node, opts) {
        if (!$node.hasClass("autocompleter-node")) {
            ...
            var html='';
            html += '<ul id="listbox"></ul>';
            $("#personlist").addClass("autocompleter-node").append(html);
            var $autocompleter = $("#personlist").next(".autocompleter").eq(0);
        ...
        if (!data.empty && data.query.length === 0) {
            ...
            if (typeof data.source === 'object') {
                ...
                .done(function (response) {
	                ...
	                var search = _search(data.query, response, data.limit);
		            if (search.length) {
		                _response(search, data);
		            }
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0; item < count; item++) {
            menu += '<li class="personbox"><a  onclick="selectperson(this)" href="javascript:;"><img src="'+list[item].src+'"></a></li>';
        }
        data.response =list;
        $("#listbox").html(menu);
    }
